...
//=================================================================================================================
//           [These sections are for folding control  in Code::Blocks]         [$Date: 2024-06-23 22:37:05 +0400 $]
...
//!
//! @file    TXLib.h
//! @brief   <b>Библиотека Тупого Художника (The Dumb Artist Library, TX Library, TXLib).</b>
//!
//!          <b> И.Р. Дединский </b><i><small> @n
...
//!          изучите ее. Простейший пример см. <a href=http://storage.ded32.net.ru/Lib/TX/TXUpdate/Doc/HTML.ru/a00001.htm>
...
//!             Пользуйтесь <a href=http://storage.ded32.net.ru/Lib/TX/TXUpdate/Doc/HTML.ru/index.htm> системой помощи
//!             онлайн.</a>
//!             @nn
...
//!
//! @warning <b>Это альфа-версия. Для использования библиотеки требуется согласование с ее автором.</b> @n
...
#define _TX_VER      _TX_v_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 173, 2024-06-23 22:37:05 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
#define _TX_VERSION  _TX_V_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 173, 2024-06-23 22:37:05 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
#define _TX_AUTHOR   _TX_A_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 173, 2024-06-23 22:37:05 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
...
//!
//! @note    Однако если @b перед включением @c TXLib.h в программу будет определен макрос <tt>#define _TX_DIB_FIX -1*</tt>
//!          или <tt>#define _TX_DIB_FIX @b -</tt> то этого переворота не будет, ось Y будет направлена снизу вверх.
//!
...
//!
//! @param   message  <i>Сообщение, выводимое в консоль через stderr, в формате printf(). Необязательно.</i>
//!
    ...
    union {                                         //
        ...
        struct                                      //        |  |                    |
            ...
            void (*padding)();                      //    +1  |  |                    |
                                                    ...
                                                    //    -*---  +====================+
    void (*unexpected_handler)();                   //           |object              |
...
int              _txPauseBeforeTermination   (HWND canvas);
...
$           for (int i = 0; i < sz.x * sz.y; i++) (*pixels)[i] = black;  //-V108
            ...
            {$ Win32::PatBlt (dc, 0, 0, sz.x, sz.y, BLACKNESS) asserted; }
        }
...
long _txOnExceptionSEH (EXCEPTION_POINTERS* exc, const char func[])
    ...
    ptrdiff_t dist = (uintptr_t) addr - (uintptr_t) IsBadReadPtr;
    if (0 <= dist && dist <= 256) {$6 return EXCEPTION_CONTINUE_SEARCH; }

              dist = (uintptr_t) addr - (uintptr_t) IsBadWritePtr;
    if (0 <= dist && dist <= 256) {$6 return EXCEPTION_CONTINUE_SEARCH; }
...
// This will never be documented, he-he. Read the source, Luke.
    ...
    #define $debug             if (1)
    #define $DEBUG             if (1)
    #define $printf(fmt, ...)  if (1)  printf (        fmt, ##__VA_ARGS__)
    #define $PRINTF(fmt, ...)  if (1) fprintf (stderr, fmt, ##__VA_ARGS__)
#else
    #define $dbg               if (0)
    #define $DBG               if (0)
    #define $debug             if (0)
    #define $DEBUG             if (0)
    #define $printf(...)       if (0)  printf (        fmt, ##__VA_ARGS__)
    #define $PRINTF(...)       if (0) fprintf (stderr, fmt, ##__VA_ARGS__)
#endif
...
#define $ppp   { $sy; txPause ("\v[%s ""%s: ""Нажмите клавишу]...", __TX_FILELINE__, __TX_FUNCTION__); }
#define $pp    { $sy; txPause ("\v[%04d %s: ""Нажмите клавишу]...", __LINE__,        __TX_FUNCTION__); }
#define $p     { $sy; txPause ("\v[%s ""%s(): Нажмите клавишу]...", __TX_FILELINE__, __func__);        }
#define $ppp_  { $sy; txPause ("\v[%s ""%s]...",                    __TX_FILELINE__, __TX_FUNCTION__); }
#define $pp_   { $sy; txPause ("\v[%04d %s]...",                    __LINE__,        __TX_FUNCTION__); }
#define $p_    { $sy; txPause ("\v[%s ""%s()]...",                  __TX_FILELINE__, __func__);        }
#define $P     ( txPause ("") )
...
#define txPause(...)                   ( _txLocCurSet(), txPause               (__VA_ARGS__) )
